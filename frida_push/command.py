...
def push_and_execute(fname, transport_id=None):
    ...
    # Add compatibility for magisk su
    kill_cmd = [ADB_PATH, "-t", transport_id, "shell", "su -c 'killall frida-server || true' || su 0 killall frida-server"]
    execute_cmd = [ADB_PATH, "-t", transport_id, "shell", "su -c  '/data/local/tmp/frida-server' || su 0 '/data/local/tmp/frida-server'"]
